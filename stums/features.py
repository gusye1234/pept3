...
def generate_prosit_feature_set(model1, model2, input_table, model1_specid, save2,
                                batch_size=2048, gpu_index=0, which_sim='SA',
                                tensor_need=False, tensor_path=None, join_unused=True):
    utils.set_seed(2022)
    ...
    with torch.no_grad():
        target_loader = DataLoader(
            dataset_manager.index_data_by_specid(target_id), batch_size=batch_size, shuffle=False)
        ...
        decoy1_loader = DataLoader(
            dataset_manager.index_data_by_specid(decoy1_id), batch_size=batch_size, shuffle=False)
        ...
        decoy2_loader = DataLoader(
            dataset_manager.index_data_by_specid(decoy2_id), batch_size=batch_size, shuffle=False)
    ...
    score_tensor = np.concatenate(
        [target_tensor, decoy1_tensor, decoy2_tensor], axis=0)
    Fs = {}
    Fs['SpecId'] = np.concatenate([target_id, decoy1_id, decoy2_id])
    Fs[which_sim] = scores

    table_data, frag_msms = dataset_manager.index_dataset_by_specid(
        Fs['SpecId'])
    ...
    Fs['Peptide'] = np.array(
        table_data['Peptide'].apply(lambda x: "_." + x + "._"))
    ...
    def add_pred(pack):
        ...
        Fs['rel_not_pred_seen'] = np.array(
            Fs['not_pred_seen']) / theoretically
        Fs['rel_not_pred_seen_b'] = np.array(
            Fs['not_pred_seen_b']) / theoretically * 2
        Fs['rel_not_pred_seen_y'] = np.array(
            Fs['not_pred_seen_y']) / theoretically * 2
        Fs['rel_pred_nonZero_b'] = np.array(
            Fs['pred_nonZero_b']) / theoretically * 2
        Fs['rel_pred_nonZero_y'] = np.array(
            Fs['pred_nonZero_y']) / theoretically * 2
        Fs['rel_pred_not_seen'] = np.array(
            Fs['pred_not_seen']) / theoretically
        Fs['rel_pred_not_seen_b'] = np.array(
            Fs['pred_not_seen_b']) / theoretically * 2
        Fs['rel_pred_not_seen_y'] = np.array(
            Fs['pred_not_seen_y']) / theoretically * 2
        Fs['rel_pred_seen_nonzero'] = np.array(
            Fs['pred_seen_nonzero']) / theoretically
        Fs['rel_pred_seen_nonzero_b'] = np.array(
            Fs['pred_seen_nonzero_b']) / theoretically * 2
        Fs['rel_pred_seen_nonzero_y'] = np.array(
            Fs['pred_seen_nonzero_y']) / theoretically * 2
        Fs['rel_pred_seen_zero'] = np.array(
            Fs['pred_seen_zero']) / theoretically
        Fs['rel_pred_seen_zero_b'] = np.array(
            Fs['pred_seen_zero_b']) / theoretically * 2
        Fs['rel_pred_seen_zero_y'] = np.array(
            Fs['pred_seen_zero_y']) / theoretically * 2
        Fs['rel_raw_nonZero_fragments'] = np.array(
            Fs['raw_nonZero_fragments']) / theoretically
        Fs['rel_raw_nonZero_b'] = np.array(
            Fs['raw_nonZero_b']) / theoretically * 2
        Fs['rel_raw_nonZero_y'] = np.array(
            Fs['raw_nonZero_y']) / theoretically * 2

        Fs['relpred_not_pred_seen2pred_nonZero_fragments'] = np.array(
            Fs['not_pred_seen']) / (np.array(Fs['pred_nonZero_fragments']) + 1e-9)
        Fs['relpred_not_pred_seen_b2pred_nonZero_b'] = np.array(
            Fs['not_pred_seen_b']) / (np.array(Fs['pred_nonZero_b']) + 1e-9)
        Fs['relpred_not_pred_seen_y2pred_nonZero_y'] = np.array(
            Fs['not_pred_seen_y']) / (np.array(Fs['pred_nonZero_y']) + 1e-9)
        Fs['relpred_pred_not_seen_b2pred_nonZero_b'] = np.array(
            Fs['pred_not_seen_b']) / (np.array(Fs['pred_nonZero_b']) + 1e-9)
        Fs['relpred_pred_not_seen_y2pred_nonZero_y'] = np.array(
            Fs['pred_not_seen_y']) / (np.array(Fs['pred_nonZero_y']) + 1e-9)

        Fs['relpred_pred_not_seen2pred_nonZero_fragments'] = np.array(
            Fs['pred_not_seen']) / (np.array(Fs['pred_nonZero_fragments']) + 1e-9)
        Fs['relpred_pred_seen_nonzero_b2pred_nonZero_b'] = np.array(
            Fs['pred_seen_nonzero_b']) / (np.array(Fs['pred_nonZero_b']) + 1e-9)
        Fs['relpred_pred_seen_nonzero_y2pred_nonZero_y'] = np.array(
            Fs['pred_seen_nonzero_y']) / (np.array(Fs['pred_nonZero_y']) + 1e-9)

        Fs['relpred_pred_seen_nonzero2pred_nonZero_fragments'] = np.array(
            Fs['pred_seen_nonzero']) / (np.array(Fs['pred_nonZero_fragments']) + 1e-9)
        Fs['relpred_pred_seen_zero_b2pred_nonZero_b'] = np.array(
            Fs['pred_seen_zero_b']) / (np.array(Fs['pred_nonZero_b']) + 1e-9)
        Fs['relpred_pred_seen_zero_y2pred_nonZero_y'] = np.array(
            Fs['pred_seen_zero_y']) / (np.array(Fs['pred_nonZero_y']) + 1e-9)

        Fs['relpred_pred_seen_zero2pred_nonZero_fragments'] = np.array(
            Fs['pred_seen_zero']) / (np.array(Fs['pred_nonZero_fragments']) + 1e-9)
    ...
    def add_kr():
        Fs['KR'] = np.array(table_data['Peptide'].apply(
            lambda x: any(map(lambda y: y in 'KR', x))))
    ...
    wanted_features = ["SpecId", 'Label', "ScanNr", 'sequence_length',
                       which_sim, "KR", "collision_energy_aligned_normed"]
    ...
    wanted_features = wanted_features + prosit_manual_feat + \
